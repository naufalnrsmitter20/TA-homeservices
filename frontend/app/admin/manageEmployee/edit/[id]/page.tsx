...
export default function EditServicesType() {
  ...
  const [ServicesTypeData, setServicesTypeData] = useState<Employee>();
  useEffect(() => {
  ...
  const HandleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.message || "Employee updated failed", { id: toastId });
        throw new Error(data.message || "Failed to update Employee");
      }
      setServicesTypeData(data.data);
      toast.success(data.message || "Employee updated successfully", { id: toastId });
      ...
      <form onSubmit={HandleSubmit} className="space-y-2 max-w-md">
        ...
        <div className="relative z-0 w-full mb-5 group">
          <input
            ...
            className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-amber-400 peer"
            placeholder=" "
            defaultValue={ServicesTypeData?.email ?? ""}
            required
          />
          <label
            htmlFor="email"
        ...
        <div className="relative z-0 w-full mb-5 group">
          <input
            ...
            className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-amber-400 peer"
            placeholder=" "
            defaultValue={ServicesTypeData?.phone ?? ""}
            required
          />
          <label
            htmlFor="phone"
        ...
        <div className="relative z-0 w-full mb-5 group">
          <input
            ...
            className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-amber-400 peer"
            placeholder=" "
            defaultValue={ServicesTypeData?.position ?? ""}
            required
          />
          <label
            htmlFor="position"
