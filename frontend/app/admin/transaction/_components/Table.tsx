...
export default function Table() {
  ...
  useEffect(() => {
    const filterProjects = () => {
      const filteredByName = transaction.filter((x) => x?.user.name?.toLowerCase().includes(searchInput.toLowerCase()));
      setFilteredUser(filteredByName);
    };
    filterProjects();
  }, [searchInput, transaction]);
  const handleSearchInput = (e: ChangeEvent<HTMLInputElement>) => {
    setSearchInput(e.target.value);
  };

  const column: TableColumn<Transaksi>[] = [
    {
      name: "No",
    ...
    {
      name: "Customer Name",
      selector: (row) => row.user.name as string,
      sortable: true,
    },
    {
      name: "Employee Name",
      selector: (row) => (row.employee ? (row.employee.name as string) : "Not Assigned"),
      sortable: true,
    },
    {
      name: "Transaction Date",
      selector: (row) => new Date(row.transactionDate).toUTCString(),
      sortable: true,
    },
    {
      name: "Invoice Number",
      selector: (row) => row.invoiceNumber as string,
      sortable: true,
    },
    {
      name: "Customer Notes",
    ...
    {
      name: "Payment Method",
      selector: (row) => row.paymentMethod as string,
      sortable: true,
    },
    {
      name: "Payment Status",
      selector: (row) => row.paymentStatus as string,
      sortable: true,
    },
    {
      name: "Total Amount",
      selector: (row) => row.totalAmount.toLocaleString("id-ID"),
      sortable: true,
    },
    {
      name: "Transaction Status",
      selector: (row) => row.transactionStatus as string,
      sortable: true,
    },
    {
      name: "Total Items",
