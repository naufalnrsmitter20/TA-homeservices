...
export default function Table() {
  ...
  const [filteredUser, setFilteredUser] = useState<ServiceType[]>(servicesType);
  useEffect(() => {
    async function Fetching() {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/services/type`, {
        method: "GET",
        cache: "no-store",
        headers: {
  ...
  const column: TableColumn<ServiceType>[] = [
    {
      name: "No",
  ...
  function editServiceTypeData(data: ServiceType) {
    router.push(`/admin/manageServiceType/edit/${data.id}`);
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          onClick={() => router.push("/admin/manageServiceType/add")}
          type="button"
